...
module.exports = {
	...
	Mutation: {
		signUpWithMagic: async (_, { profile }) => {
			// Check if user already exists based on a unique property (like email)
			...
			if (!user) {
				...
				// If user exists, update only the properties you want
				// For example, only update the img_url if it doesn't exist
				if (!user.img_url) {
					user.img_url = profile.picture;
				}
				// ... add logic for other fields you want to conditionally update
			}
		...
		async updateUserImgUrl(_, { img_url }, context) {
			...
			if (user.username === "--" || user.walletAddress) {
				const uData = await UserData.findOne({
					walletAddress: user.walletAddress,
				});
				if (!uData) {
					throw new Error("User not found");
		...
		async followProfile(_, { username }, context) {
			...
			if (
				data.followers.find((follower) => follower.username === user.username)
			) {
				data.followers = data.followers.filter(
					(follower) => follower.username !== user.username
				);
				user.following = user.following.filter(
					(following) => following.username !== data.username
				);
			} else {
				data.followers.push({
					username: user.username,
